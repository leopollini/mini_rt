...
int	initw(t_window *win)
{
	...
	win->do_exit = 0;
	rft_load_scene(win);
	win->win = mlx_new_window(win->mlx, win->size.x, win->size.y,"mini_rt lol");
	win->skybox.img.img = mlx_xpm_file_to_image(win->mlx, "skybox/universe.xpm", &win->skybox.size.x, &win->skybox.size.y);
	win->skybox.img.addr = mlx_get_data_addr(win->skybox.img.img, &win->skybox.img.bps, &win->skybox.img.ll, &win->skybox.img.en);
	win->img.img = mlx_new_image(win->mlx, win->size.x, win->size.y);
	win->img.addr = mlx_get_data_addr(win->img.img, &win->img.bps, &win->img.ll, &win->img.en);
	return (0);
}
